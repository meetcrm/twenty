...
PODMAN_NETWORK=twenty_network

ensure-podman-network:
	podman network inspect $(PODMAN_NETWORK) >/dev/null 2>&1 || podman network create $(PODMAN_NETWORK)

postgres-on-podman:
	make ensure-podman-network
	podman run -d --network $(PODMAN_NETWORK) \
	--name twenty_pg \
	-e POSTGRES_USER=postgres \
	-e POSTGRES_PASSWORD=postgres \
	-e ALLOW_NOSSL=true \
...
redis-on-podman:
	make ensure-podman-network
	podman run -d --network $(PODMAN_NETWORK) --name twenty_redis -p 6379:6379 redis/redis-stack-server:latest

clickhouse-on-podman:
	make ensure-podman-network
	podman run -d --network $(PODMAN_NETWORK) --name twenty_clickhouse -p 8123:8123 -p 9000:9000 -e CLICKHOUSE_PASSWORD=devPassword clickhouse/clickhouse-server:latest
...
up: postgres-on-podman redis-on-podman clickhouse-on-podman grafana-on-podman opentelemetry-collector-on-podman
	@echo "✅ Все сервисы запущены через Podman"